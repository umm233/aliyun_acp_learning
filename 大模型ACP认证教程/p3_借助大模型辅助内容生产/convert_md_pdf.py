...
def convert_single_md_to_pdf_and_png_ffx(md_file_path, output_base_dir="./Marp/"):
    ...
    # 构建Marp CLI命令
    command = ["marp", "--html", "--allow-local-files", "--browser", "firefox",
               "-o", os.path.join(output_dir, filename + ".pdf"),
               md_file_path]
...
def convert_single_md_to_pptx_and_png_ffx(md_file_path, output_base_dir="./Marp/"):
    """
    将单个MD文件转换为PDF和PNG格式，PDF和PNG文件将保存在由MD文件名决定的子目录中。
    ...
    # 构建Marp CLI命令
    command = ["marp", "--html", "--allow-local-files", "--browser", "firefox",
               "-o", os.path.join(output_dir, filename + ".pptx"),
               md_file_path]
    try:
        # 执行命令，将MD文件转换为PPTXs
        print(command)
...
if __name__ == "__main__":
    ...
    try:
        ...
        convert_single_md_to_pptx_and_png_ffx(args.file_path, args.output_dir)
